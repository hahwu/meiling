...
//        array_push($data, $total);
    ...
    public function sycm_year($sycm='')
    {
        ...
        for($i=0;$i<count($data);$i++){
            ...
            $data[$i]['gap'] = round($data[$i]['actsale'] - $data[$i]['starget']);
...
//        array_push($data, $total);
    ...
    public function tx($date=''){
        ...
        for($i=0;$i<count($data);$i++){
            $data[$i]['zfzhl'] = ($data[$i]['zfzhl']*100)."%";
            $data[$i]['xszk'] = ($data[$i]['xszk']*100)."%";
        }
        $data2['data'] = $data;
        $data2['date'] = $date;
        $json = json_encode($data2,JSON_UNESCAPED_UNICODE);
    ...
    public function stock($date=''){
        ...
        $data = $ZTC->select();
        for($i=0;$i<count($data);$i++){
            $data[$i]['stockr'] = ($data[$i]['stockr']*100)."%";
            $data[$i]['saler'] = ($data[$i]['saler']*100)."%";
    ...
    {
        header("Content-type: text/html; charset=utf-8");
        $data = M('ztc')->where("store = '中端商务男装'")->delete();
        $data = M('pxb')->where("date = '2017-07-04'")->order(array('team'))->select();
        $three_data = M('pxb')->field('store,project,sum(consume) as consume,sum(click) as click,sum(reveal) as reveal,sum(storecollect) as storecollect')->where("date <= '2017-07-06' and date >= '2017-07-04'")->group('store,project')->order(array('team','store'=>'desc','project'=>'desc'))->select();
